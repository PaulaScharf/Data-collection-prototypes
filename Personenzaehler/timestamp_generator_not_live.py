...
def run_step_gui():
    ...
    while not stop_recording:
        root.update()
        if init_form_view:
            gui_step.init_form_view(root,"whatever",webcam_in_folder(gui_file_dir))
...
def detect(save_img=False):
    ...
    file = open(str(save_dir / "labels.txt"), "a")
    file.write("label,timestamp\n")
    file.write("stop_step,"+str(t0))
    file.close()
            ...
            if len(det):
                ...
                if has_overstepped_center:
                    ...
                    if last_recording_of_step>0 and (t1-last_recording_of_step)>0.6:
                        ...
                        if start_step > 0:
                            file = open(str(save_dir / "labels.txt"), "a")
                            file.write("\nstop_step,"+str(last_recording_of_step))
                            file.close()
                        currently_stepping = False
                        start_step = 0
