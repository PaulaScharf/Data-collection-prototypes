...
startpoint_0 = pygame.math.Vector2(0, 175)
startpoint_1 = pygame.math.Vector2(0, 525)
startpoint_2 = pygame.math.Vector2(0, 825)
...
for line in lines:
    ...
    if int(type) == 3:
        imp = pygame.image.load('data/2/yolo-result/' + line_split[0]).convert()
        ...
        if(prev_timestamp!=0):
            ...
            prev_timestamp = current_timestamp
            FPSCLOCK.tick(30)
    elif line_split[0] != 'Time' and ((float(line_split[1]) > 30) and float(line_split[1]) < 160):
        ...
        if int(line_split[3]) > -1 and float(line_split[2])/10 < 250:
